...
const DECORATOR_KEYWORDS = new Set([
  'Component', 'NgModule', 'Injectable', 'Directive', 'Pipe', 'Service',
  'Input', 'Output', 'Inject', 'Optional', 'Self', 'SkipSelf', 'Host',
])
...
function walk(dir: string, cwd: string, out: ProviderEntry[]) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      for (const match of content.matchAll(/@(?:Injectable|Service)\s*(?:\(\s*\{?\s*(?:providedIn:\s*['"`](\w+)['"`])?\s*\}?\s*\))?\s*\n?\s*(?:export\s+)?class\s+(\w+)/g)) {
      ...
      for (const match of content.matchAll(/(?:(?:private|protected|public|readonly)\s+)*(\w+)\s*=\s*inject\s*(?:<[^>]*>)?\s*\(\s*(\w+)/g)) {
        out.push({
          token: match[2],
          source: match[1],
          file: relPath,
      ...
      for (const match of content.matchAll(/@Inject\(\s*(\w+)\s*\)\s*(?:private|protected|public|readonly|\s)*(\w+)/g)) {
        out.push({
          token: match[1],
          source: match[2],
          file: relPath,
